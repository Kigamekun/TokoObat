...
const DashboardLayout = ({ children, user, onLogout }) => {
  ...
  const navigation = [
    ...
    { name: 'Transactions', href: '/transactions', icon: ShoppingCart, current: location.pathname === '/transactions' },
    { name: 'Medicine Catalog', href: '/medicines', icon: Package, current: location.pathname === '/medicines' },
  ...
  if (user?.role === 'admin') {
    navigation.push(
      { name: 'User Management', href: '/users', icon: Users, current: location.pathname === '/users' },
          ...
          <div className="flex flex-col h-0 flex-1 bg-white border-r border-gray-200 shadow-sm">
            ...
            <div className="flex-1 flex flex-col pt-5 pb-4 overflow-y-auto">
              <div className="flex items-center flex-shrink-0 px-6 mb-8">
                <div className="bg-teal-600 rounded-lg w-10 h-10 flex items-center justify-center">
                  <Shield className="w-6 h-6 text-white" />
                </div>
                <div className="ml-3">
                  <h1 className="text-lg font-semibold text-gray-900">Mitra Toko Obat</h1>
                  <p className="text-xs text-teal-600 font-medium">JGroup</p>
                </div>
              </div>
              ...
              <nav className="flex-1 px-3 space-y-1">
                {navigation.map((item) => {
                  ...
                  return (
                    ...
                    >
                      <Icon className={`${item.current ? 'text-teal-500' : 'text-gray-400 group-hover:text-gray-500'} mr-3 h-5 w-5`} />
                      {item.name}
                    </Link>
                  );
                })}
            ...
            <div className="flex-shrink-0 flex border-t border-gray-200 p-4">
              <div className="flex items-center w-full">
                ...
                <div className="ml-3 flex-1">
                  <p className="text-sm font-medium text-gray-700" data-testid="user-name">{user?.fullName}</p>
          ...
          <div className="relative flex-1 flex flex-col max-w-xs w-full bg-white">
            ...
            <div className="flex-1 h-0 pt-5 pb-4 overflow-y-auto">
              <div className="flex-shrink-0 flex items-center px-4 mb-8">
                ...
                <div className="ml-2">
                  <h1 className="text-base font-semibold text-gray-900">Mitra Toko Obat</h1>
                  <p className="text-xs text-teal-600 font-medium">JGroup</p>
                </div>
              </div>
              <nav className="px-2 space-y-1">
                {navigation.map((item) => {
                  ...
                  return (
                    ...
                    >
                      <Icon className={`${item.current ? 'text-teal-500' : 'text-gray-400'} mr-4 h-6 w-6`} />
                      {item.name}
                    </Link>
                  );
                })}
